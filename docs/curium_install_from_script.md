Curium MasterNode Install from script (10 minutes)
-------
...
- Open your Curium wallet on your desktop.
...
`MN1 148.124.58.33:18745 7xxxx(you get the idea)xxx 6a66ad6011ee363c2d97da0b0ef43137b478aa73b4b906b0 0`

Put your data correctly, **save it** and close.

Go to Curium Wallet, Click `Settings`, Check `Show Masternodes Tab`.

Save and restart your Curium wallet.
...
Now lets download the script, copy command into the VPS command line and hit enter: ``wget https://raw.githubusercontent.com/curiumcrypto/curium-docs/master/scripts/masternode.sh``
...
Please note, the script will move curiumd and curium-cli binaries to /usr/bin folder, so you don't need to navigate to curium/src folder anymore, you can run the commands without "./" on any place now.
...
`nano /root/.curiumcore/curium.conf`
...
- If get error: incorrect rpcuser or rpcpassword (authorization failed) you need to kill the process first with: pkill -9 curiumd
Then save and exit using CTRL+X, Y and Enter, and restart daemon with:
`curiumd -daemon`
Now test with:
`curium-cli getinfo`  or  `curium-cli getblockcount`

- If after start alias and when check with curium-cli masternode status on your VPS isn't showing as "Masternode successfully started", go to your data folder of local wallet and delete files: mncache.dat and mnpayments.dat, restart local wallet, wait full sync and start the alias again.
